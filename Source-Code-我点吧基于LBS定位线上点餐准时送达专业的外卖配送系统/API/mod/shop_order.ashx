...
public class order :API_Base, IHttpHandler {
        ...
        try
        {
            switch (Action)
            {
                case "carttoorder"://购物车生成订单,计算出当前金额和运费等返回()
                        ...
                        else
                        {
                            ...
                            foreach (DataRow dr in storeDT.Rows)
                            {
                                ...
                                else
                                {
                                    ...
                                    storeMod.prodt = dt;
                                    storeMod.faredt = GetFareDT(dt);//为每个店铺生成运费
                    ...
                    break;
                case "createorder"://选定好送货与地址后生成支付单(返回支付单)-->选择支付方式
                    {
                        //暂时不计算运费(运费需要确定好样式)
                        ...
                        {
                            M_Payment payMod = OrderToPayment(mu, receMod, ids);
                            retMod.result = JsonConvert.SerializeObject(payMod);
                            retMod.retcode = M_APIResult.Success;
                        }
                    }
                    break;
                ...
                case "list"://我的订单
                    {
                        DataTable dt = orderBll.U_SelByUserID(mu.UserID);
    ...
    private M_Payment OrderToPayment(M_UserInfo mu, M_UserRecei receMod,string ids)
    {
    ...
    private double GetFarePrice(DataTable storecartDT, int storeid)
    {
        ...
        string selectedVal = Req("fare_" + storeid);//前台选定的快递方式,后台重新计价
