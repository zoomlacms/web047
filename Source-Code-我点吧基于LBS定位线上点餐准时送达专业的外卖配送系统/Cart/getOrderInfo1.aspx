<%@ Page Language="C#" AutoEventWireup="true" CodeFile="getOrderInfo1.aspx.cs" Inherits="test_getOrderInfo1" MasterPageFile="~/Cart/order1.master" EnableViewStateMac="true" Debug="true" %>
<asp:Content runat="server" ContentPlaceHolderID="head">
    ...
    <style>
		...
		.weui_label { font-weight:normal; margin-bottom:0;}
    </style>
</asp:Content>
<asp:Content runat="server" ContentPlaceHolderID="Content">
    ...
    <div class="gray_border">
        ...
        <div class="bordered" runat="server" id="Address_Div" style="background:#fff;">
                ...
                <asp:Repeater runat="server" ID="AddressRPT">
                    <ItemTemplate>
                        <li id="addli_<%#Eval("ID") %>">
                            ...
                            <span>
                                <%#GetIsDef() %>
                                <a href="address1.aspx?id=<%#Eval("ID") %>&ids=<%= Request.QueryString["ids"] %>&ProClass=<%= Request.QueryString["ProClass"] %>&remark=<%= Request.QueryString["remark"] %>">修改</a>
            ...
            <a href="address1.aspx?ids=<%= Request.QueryString["ids"] %>&ProClass=<%= Request.QueryString["ProClass"] %>&remark=<%= Request.QueryString["remark"] %>" class="btn btn-primary btn-xs">添加新地址</a>
        </div>
...
" />
        ...
        <div class="bordered" style=" padding:0;">
            <div class="">
                ...
                <i id="payPriceId" class="fa fa-rmb" style="display:<%# Eval("AllMoney","{0:F2}")=="0.00"?"none":""%>"><%#Eval("AllMoney","{0:F2}") %></i>
                <%#GetAllMoney_Json() %>
                </span>
                ...
                <div class="weui_cells weui_cells_form fare_div" style="display:none;">
                	<div class="weui_cell">
                        <div class="weui_cell_hd"><label class="weui_label">配送方式</label></div>
                ...
                <ul>
                    ...
                    <li>
                        <div hidden>
                        	<button type="button" class="btn btn-danger btn-block" onclick="$('#arrive_div').toggle();"><i class="fa fa-plus-circle"></i> 使用优惠券抵消部分总额</button>
                        </div>
                        <div id="arrive_div">
                        	<div class="weui_cells weui_cells_form">
                            	<div class="weui_cell">
                                    <div class="weui_cell_hd"><label class="weui_label">优惠券号</label></div>
                                    <div class="weui_cell_bd weui_cell_primary">
                                    	<asp:TextBox runat="server" ID="Arrive_T" class="form-control input-sm" placeholder="没有优惠券可不填"/>
                                    </div>
                                </div>
                                <div class="weui_cell">
                                    <div class="weui_cell_hd"><label class="weui_label">密码</label></div>
                                    <div class="weui_cell_bd weui_cell_primary">
                                    	<asp:TextBox runat="server" ID="Arrive_Pwd" class="form-control input-sm" placeholder="密码"/>
                                    </div>
                                </div>
                            </div>
                            <div class="weui_btn_area">
                            	<button type="button" class="weui_btn weui_btn_primary" onclick="ArriveCheck();">使用优惠券</button>
                            </div>
                        </div>
                    </li>
                    <li id="point_li" style="display:none;">
                        <div hidden>
                        	<button type="button" class="btn btn-info btn-block" onclick="$('.point_div').toggle();"><i class="fa fa-plus-circle"> 使用积分抵扣</i></a>
                        </div>
                        <div id="point_body" runat="server" class="point_div" style="margin-left:0;">
                        	<div class="weui_cells weui_cells_form">
                            	<div class="weui_cell">
                                    <div class="weui_cell_hd"><label class="weui_label">积分抵扣</label></div>
                                    <div class="weui_cell_bd weui_cell_primary">
                                        <asp:TextBox runat="server" ID="Point_T" Text="" onkeyup="checkMyPoint(this);" CssClass="form-control input-sm num"/>
                                    </div>
                                </div>
                            </div>
                            <h5 style="text-align:center;">您当前有<asp:Label ID="Point_L" runat="server"></asp:Label>个积分,本次可用<span id="usepoint_span" class="r_red"></span>个积分<br/><span style="color:#c00;">注：100积分=1元</span></h5>
                        </div>
        ...
        <div class="alert alert-info proclass<%= Request.QueryString["ProClass"] %>" role="alert" style="margin-bottom:0; display:none;">我点吧商城外卖20元起送哦！</div>
        <div class="total_div">
            <span class="total">应付总额：<i runat="server" id="totalmoney_i" class="fa fa-rmb">0.00</i></span>
            <asp:Button runat="server" CssClass="btn btn-danger" ID="AddOrder_Btn" Text="提交订单" OnClientClick="disBtn(this,5000);" OnClick="AddOrder_Btn_Click" />
        </div>
...
<script type="text/javascript">
    $().ready(function (e) {
        $(".proclass2").show();
		if("<%= Request.QueryString["ProClass"] %>"=="3")
		{
			$(".fare_div").show();
			$("#point_li").show();
		}
		else if("<%= Request.QueryString["ProClass"] %>"=="2")
		{
			$("#point_li").hide();
			$(".fare_div").show();
		}
    ...
    function IsDisBtn() {
        ...
        if (parseFloat($("#totalmoney_span1").text()) < 20 && "<%= Request.QueryString["ProClass"]%>" == "2") { flag = true;}
        if (flag)
