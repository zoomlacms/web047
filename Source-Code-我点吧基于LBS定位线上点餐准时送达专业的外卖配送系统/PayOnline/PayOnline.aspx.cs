    ...
    public partial class PayOnline : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                Rurl_Href.NavigateUrl = "/User/Order/OrderList1.aspx";//返回我的订单
                ...
                if (string.IsNullOrEmpty(PayMethod))
                {
                    ...
                    switch ((M_PayPlat.Plat)payPlat.PayClass)//现仅开通 12:支付宝即时到账和支付宝网银服务,15支付宝网银服务(上方代码中处理),银币与余额服务
                    {
                        ...
                        case M_PayPlat.Plat.WXPay:
                            #region 微信扫码支付
                            {
								Response.Redirect("/api/pay/wxpay_mp.aspx?PayNo=" + pinfo.PayNo+"&appid="+Request["appid"]);
                                string redirect_uri = HttpUtility.UrlEncode("http://www.wodian8.com/Payonline/wxpayonline.aspx?PayNo=" + pinfo.PayNo);
                                string wxurl = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx8c5c2dc7b10a36f3&redirect_uri=" + redirect_uri + "&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect";
                ...
                DataTable ordertable1 = orderBll.GetOrderbyOrderNo(pinfo.PaymentNum);

                //显示金额信息
                LblPayMoney.Text = pinfo.MoneyReal.ToString("F2") + " 元";//应付金额
                string priceJson = orderCom.GetTotalJson(ordertable1);//多虚拟币支付,暂取消
        ...
        private void PayByVirtualMoney(string payMethod, M_Payment payMod)
        {
            ...
            List<M_OrderList> orderList = OrdersCheck(payMod);
        ...
        private int GetPrintModelID(int orderid)
        {
            ...
            {
                DataTable pdt = tlpBll.Sel();
                pdt.DefaultView.Sort = "ID ASC";
                pdt = pdt.DefaultView.ToTable();
                if (pdt.Rows.Count > 0)
                {
                    modelid = DataConvert.CLng(pdt.Rows[0]["ID"].ToString());
                }
            }
            return modelid;
        }
        private int GetPrintDevice(int orderid)
        {
            ...
            {
                DataTable pdt = devBll.Sel();
                pdt.DefaultView.Sort = "ID ASC";
                pdt = pdt.DefaultView.ToTable();
                if (pdt.Rows.Count > 0)
                {
                    deviceid = DataConvert.CLng(pdt.Rows[0]["ID"].ToString());
                }
            }
            return deviceid;
        }
