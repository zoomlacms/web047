        ...
        <ZL:ExRepeater ID="Deposit_RPT" runat="server" OnItemCommand="Deposit_RPT_ItemCommand" PageSize="10" PagePre="<tr id='page_tr'><td><input type='checkbox' id='chkAll'/></td><td colspan='10' id='page_td'>" PageEnd="</td></tr>">
            <ItemTemplate>
             ...
             <%--   <td><%#Eval("Y_ID") %></td>--%>
                    ...
                    <td>
                        <a href="javascript:;" title="查看下级订单" onclick="ShowOrder(<%#Eval("UserID") %>);">
                            <i class="fa fa-dollar"></i><%#Eval("money","{0:f0}") %>
                        </a>
                        <input type="hidden" name="price_hid" value="<%#Eval("money") %>" />
                    </td>
...
<%--            <tr>
    ...
    <asp:Button ID="CheckDepos_B" CssClass="btn btn-primary" runat="server" OnClick="CheckDepos_B_Click" OnClientClick="return confirm('要确认选定出金吗?');" Text="批量确认" Visible="false" />
    <div class="alert alert-info margin_t5">
        <div><span class="sp_l">余额提现：</span>确认后会自动转入用户账户,拒绝后会自动返还</div>
        <div><span class="sp_l">银行卡提现：</span>确认后需要手动转账,拒绝后自动返还</div>
    </div>
<%--    <div class="alert alert-info margin_t5">拒绝出金后,请手动为用户增加金额</div>--%>
</asp:Content>
<asp:Content runat="server" ContentPlaceHolderID="ScriptContent">
    <style type="text/css">
        .sp_l {display:inline-block;min-width:90px;text-align:right;}
    </style>
    ...
    <script>
        ...
        function ShowOrder(uid) {
            ShowComDiag("../Shop/OrderList.aspx?puid=" + uid, "下级订单");
        }
